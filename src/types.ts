...
function parseVersionFromProjectConfig(project: ProjEnvProject): string | null {
    try {
        ...
        if (versionMatch && versionMatch[1]) {
            ...
            console.log(
                `[ProjectInfo] Parsed version from config: ${version} (normalized: ${normalized})`,
            );
            ...
            if (!availableVersions.includes(normalized)) {
                console.warn(
                    `[ProjectInfo] Parsed version ${normalized} not found on system (available: ${availableVersions.join(
                        ', ',
                    )})`,
                );
                return null;
...
export function toProjectInfo(
    ...
    if (!version || version === 'unknown' || version.trim() === '') {
        ...
        if (configVersion) {
            ...
            console.log(
                `[ProjectInfo] Using version from config: ${version} (set in ProjEnvProject)`,
            );
        } else {
